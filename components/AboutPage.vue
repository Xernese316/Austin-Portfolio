...
onUnmounted(() => {
  clearInterval(timer); // Clean up timer to prevent memory leaks
});


// --- REAL-TIME WEATHER ---
// This securely gets the configuration, including your API key
const config = useRuntimeConfig();
const apiKey = config.public.openweatherApiKey;

// Coordinates for Denver, CO
const lat = 39.7392;
const lon = -104.9903;
// The apiKey is safely inserted here from the runtime config
const apiUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=imperial`;

// useFetch handles state (pending, error) and data fetching
const { data: weather, pending, error } = useFetch(apiUrl, {
  lazy: true, // Doesn't block page navigation
});

// Computed properties to safely access weather data and prevent errors
const temperature = computed(() => Math.round(weather.value?.main?.temp) ?? 'N/A');
const condition = computed(() => weather.value?.weather[0]?.main ?? 'Loading...');
const weatherIcon = computed(() => {
  const iconCode = weather.value?.weather[0]?.icon;
  // This map translates weather codes from the API to icons you have available
  const iconMap = {
    '01d': 'heroicons:sun-20-solid',
    '01n': 'heroicons:moon-20-solid',
    '02d': 'heroicons:cloud-sun-20-solid',
    '02n': 'heroicons:cloud-moon-20-solid',
    '03d': 'heroicons:cloud-20-solid',
    '03n': 'heroicons:cloud-20-solid',
    '04d': 'heroicons:cloud-20-solid',
    '04n': 'heroicons:cloud-20-solid',
    '09d': 'heroicons:cloud-rain-20-solid',
    '09n': 'heroicons:cloud-rain-20-solid',
    '10d': 'heroicons:cloud-rain-20-solid',
    '10n': 'heroicons:cloud-rain-20-solid',
    '11d': 'heroicons:bolt-20-solid',
    '11n': 'heroicons:bolt-20-solid',
    '13d': 'heroicons:snow-20-solid',
    '13n': 'heroicons:snow-20-solid',
    '50d': 'heroicons:bars-3-bottom-left-20-solid',
    '50n': 'heroicons:bars-3-bottom-left-20-solid',
  };
  return iconMap[iconCode] || 'heroicons:sparkles-20-solid'; // A default icon
});
</script>
      ...
      <div class="grid md:grid-cols-2 gap-12 items-center">
        ...
        <div>
          ...
          <div class="mb-6 bg-white dark:bg-gray-900 p-4 rounded-lg shadow-sm flex items-center">
            <img 
              src="~/assets/images/cu-denver-logo.png" 
          ...
          <div class="bg-white dark:bg-gray-900 p-4 rounded-lg shadow-sm flex items-center">
            <img 
              src="~/assets/images/pune-university-logo.png" 
        ...
        <div class="bg-gradient-to-br from-blue-900 via-indigo-900 to-slate-900 text-white p-6 rounded-2xl shadow-2xl min-h-[180px]">
          ...
          <div v-else-if="weather" class="flex flex-col h-full">
            <div class="flex justify-between items-start">
              <div class="flex items-center space-x-4">
                <Icon :name="weatherIcon" class="text-7xl text-slate-300" />
                <div class="text-6xl font-bold">{{ temperature }}°</div>
              </div>
              <div class="flex items-center text-sm bg-black/20 px-2 py-1 rounded-full">
                <Icon name="mdi:map-marker" class="mr-1" />
                Denver, CO
              </div>
            </div>
            <div class="mt-2 ml-2">
              <p class="text-2xl font-semibold">{{ condition }}</p>
              <p class="text-sm text-slate-300">Local Time: {{ currentTime }}</p>
            </div>
            <div class="mt-auto ml-2 text-sm text-slate-400">
                Living here since August 2023
            </div>
          </div>
        </div>
      </div>
